...
// Setup Header
$page_title = "Reference History | NGA Educator";
$header_title = "My Resource History";
...
<div class="glass rounded-[32px] border border-slate-200/50 dark:border-slate-800/50 p-8 shadow-sm">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            <h2 class="text-2xl font-black text-slate-900 dark:text-white">Resource Log</h2>
            <p class="text-slate-500 dark:text-slate-400 font-medium">A complete historical log of your academic references.</p>
        </div>
        ...
        <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-800/50 p-2 rounded-2xl">
            <div class="px-4 py-2 bg-white dark:bg-slate-800 rounded-xl shadow-sm text-xs font-bold text-slate-900 dark:text-white">
                Total Logs: <?php echo count($history); ?>
            </div>
        </div>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-[10px] uppercase tracking-widest text-slate-500 dark:text-slate-400 font-black">
                    <th class="p-4 pl-6">Resource Details</th>
                ...
                <?php if (count($history) > 0): ?>
                    ...
                    ?>
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all duration-300">
                            <td class="p-4 pl-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-14 bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 rounded-lg flex items-center justify-center flex-shrink-0 shadow-inner">
                                        ...
                                        <?php else: ?>
                                            <i class='bx bxs-book-bookmark text-slate-400 text-xl'></i>
                                        <?php endif; ?>
                                    </div>
                                    <div>
                            ...
                            <td class="p-4 text-slate-600 dark:text-slate-400 font-bold">
                                <?php echo $item['return_date'] ? date('M j, Y', strtotime($item['return_date'])) : '<span class="text-slate-400 italic">Active / Pending</span>'; ?>
                            </td>
                            <td class="p-4">
                                <?php if ($item['status'] === 'Returned'): ?>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-green-100/50 dark:bg-green-500/10 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-500/20 rounded-full text-[10px] font-black uppercase tracking-wider">
                                        <i class='bx bx-check-double'></i> Returned
                                    </span>
                                <?php elseif ($item['status'] === 'Pending'): ?>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-100/50 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-500/20 rounded-full text-[10px] font-black uppercase tracking-wider">
                                ...
                                <?php else: ?>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-100/50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 rounded-full text-[10px] font-black uppercase tracking-wider">
                                        <i class='bx bx-bookmark'></i> Active
                                    </span>
                                <?php endif; ?>
                            </td>
                        </tr>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="p-12 text-center">
                            ...
                            <p class="text-sm text-slate-500 mt-1">Your resource history will appear here once you start using the library.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
